...
from sim.utils.robot.robot_pc_transformations import transform_gs_xarm_gripper
...
def control_robot_with_gripper(params_path, total_mask_path, qpos=[0, -45, 0, 30, 0, 75, 0], gripper_openness=800):
    temp_dir = Path()
    sp = GSProcessor()
    ...
    params_full = transform_gs_xarm_gripper(qpos, gripper_openness, params_full, total_mask_full, sample_robot=None)
    
    sp.save(params_full, 'log/gs/temp_save/full.splat')
    sp.visualize_gs(['log/gs/temp_save/full.splat'], transform=True, merged=False, axis_on=True)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--construct', action='store_true', help='Whether to construct the gripper scene gs')
    parser.add_argument('--visualize', action='store_true', help='Whether to visualize the gripper scene')
